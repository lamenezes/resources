...
def test_resource_manager_build_model(gist_resource_manager):
    ...
    assert isinstance(resource, gist_resource_manager._resource_class)


@mock.patch('resources.manager.ResourceManager.client')
...
def test_resource_manager_create_or_update(mock_client, gist_resource_manager, gist_id, gist_data):
    ...
    gist = gist_resource_manager.create_or_update(pk=gist_id, **gist_data)
    assert gist.id == gist_id

    mock_client.put.assert_called_with(pk=gist_id, **gist_data)
